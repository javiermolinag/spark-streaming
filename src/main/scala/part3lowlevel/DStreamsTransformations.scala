...
object DStreamsTransformations {
  ...
  def readPeople(): DStream[Person] = ssc
  ...
  */
  ...
  // foreach rdd
  def saveToJsonFromDS(): Unit = readPeople().foreachRDD { rdd =>
  ...
  // foreach rdd
  def saveToJsonFromDF(): Unit = readPeople().foreachRDD { rdd =>
    val df: DataFrame = spark.createDataFrame(rdd)
    df.printSchema()
    df.show()
    df.write.mode("append").json("src/main/resources/data/people/people.json")
  }
